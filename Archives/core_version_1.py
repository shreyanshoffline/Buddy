...
def process_message(user_input, message_history, on_event=None):
...
def send_and_save_message(conversation_id, user_text, on_event=None):
    """Handles the database saving and talks to the AI."""

    # 1. Keep the existing conversation context before saving the new turn.
    ...
    # 3. Load the full history for model context, including system instructions.
    history = new_message_history() + _sanitize_history_for_model(previous_history)

    # 4. Get the response from your existing AI function
    result = process_message(user_text, history, on_event=on_event)

    # 5. Save the AI's response to the database, including the thought process metadata.
    #    Note: this is UI display info (plan/tools/stats), not real model tool_calls,
    #    so it goes in the `metadata` column, not `tool_calls`.
    ...
    # 6. Persist the chat title, either from the model or auto-generated fallback.
