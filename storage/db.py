...
def get_db_path():
    global _DB_PATH
    if _DB_PATH is None:
        configured_path = os.environ.get("BUDDY_DB_PATH")
        path = Path(configured_path).expanduser() if configured_path else Path(_default_db_path())
...
def _connect():
    ...
    try:
        # SQLite recreates a missing file as an empty database. Reconcile the
        # schema before every use so a running app recovers instead of failing
        # later with "no such table" after the file was removed or replaced.
        init_db(conn)
        yield conn
        conn.commit()
    except Exception:
...
def init_db(conn=None):
    ...
    try:
        ...
        extra_profile = {
            ...
            "thinking_level_manual": "INTEGER DEFAULT 0",
            "credits_low": "INTEGER DEFAULT 0",
        ...
        for name, spec in extra_profile.items():
            if name not in cols:
                conn.execute(f"ALTER TABLE user_profile ADD COLUMN {name} {spec}")
...
def update_profile(**fields):
    ...
    if "thinking_level_manual" in fields:
        fields["thinking_level_manual"] = 1 if fields["thinking_level_manual"] else 0
    if "credits_low" in fields:
        fields["credits_low"] = 1 if fields["credits_low"] else 0
    if not fields:
        return
